 ...
 * @author Dave Syer
 */
@ConditionalOnBean({ ZookeeperDiscoveryClientConfiguration.class })
@ConditionalOnProperty(value = "spring.cloud.config.discovery.enabled", matchIfMissing = false)
public class ZookeeperDiscoveryClientConfigServiceAutoConfiguration {
	...
	public void init() {
		if (this.context.getParent() != null) {
			if (/*this.context.getBeanNamesForType(ZookeeperServiceDiscovery.class).length > 0
					&& */this.context.getParent()
							.getBeanNamesForType(ZookeeperServiceDiscovery.class).length > 0) {
				// If the parent has a EurekaClient as well it should be shutdown, so the
				// local one can register accurate instance info
				this.context.getParent().getBean(ZookeeperServiceDiscovery.class).shutdown();
			}
		}
	}
